...
describe('pointsFor — 0-0 default rule', () => {
  it('scores a missing bet as 0-0', () => {
    ...
    expect(pointsFor(undefined, finished(0, 2))).toBe(0);   // 0-0 predicts draw; 0-2 is away win → 0
    expect(pointsFor(undefined, finished(1, 1))).toBe(1);   // 0-0 predicts draw; 1-1 is draw → 1
  });
